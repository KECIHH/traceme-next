...
- MVP 只提供一个业务 API：`POST /api/trips/generate`。
...
- API Route：`POST /api/trips/generate` 处理请求、返回 `TripGenerationResponse`。
...
`src/app/api/trips/generate/route.ts` 负责：
